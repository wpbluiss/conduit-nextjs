...
## Round Brand-5 — Conduit visual overhaul v2 (2026-05-08)

Branch: `feat/conduit-visual-overhaul-2026-05-08`. Quality bar:
"would Geist Studio (Anthropic) or Rauno Freiberg (Linear) ship
this." Brand-3 + Brand-4 marketing rewrites get fully replaced; the
Console (everything under /app/*) is left alone.

### What this is not
- Not a copy edit. The information architecture changed.
- Not a token tweak. New token system, new fonts, new motion.
- Not the founder's blog. All "21 years old", "no CS degree", "one
  founder on a laptop", "built in public" framing is gone. West
  Palm Beach survives in the footer copyright line — that's it.

### Phase 0 — references study
- `docs/conduit-visual-references.md` synthesises Anthropic /
  Stripe / Linear (+ /method) / Vercel / Mercury into a Conduit
  design DNA: ember radial top-right, dark-first, single-accent
  discipline, ring-glow over box-shadow, hybrid illustration +
  screenshot, manifesto-voice copy.

### Phase 1 — design system v2 (`feat(design): conduit token system v2 + dark mode`)
- `src/lib/design-system/conduit-tokens.ts` is the TS source of
  truth. CSS counterparts in `globals.css` (additive — every old
  /app token preserved unchanged).
- New scales: ink (canvas/surface/subtle/elevated), cream
  (primary/soft/mute/faint), edge (subtle/default/strong), ember
  (50/100/300/500/700/900), semantic (success/warning/danger/info).
- Type pairing: Fraunces (display) × Inter (UI) × JetBrains Mono
  (code/captions). Mono added via `next/font/google`.
- Motion: `--conduit-ease-out-quart` cubic-bezier(0.25,1,0.5,1) is
  the universal easing. Durations 100/200/400/800/1200ms.
- Dark-mode shadow recipe: lift via inset 1px ring + soft outer
  ember-glow. Never box-shadow.
- Phosphor (1.5 stroke) installed. Lucide is no longer imported in
  any marketing surface.
- Skip-to-main accessibility link added at body start.

### Phase 2 — nav + footer (`feat(shell): nav + footer rebuild`)
- New ember-filled-circle mark replaces the orange "C" SVG on
  marketing surfaces.
- Sticky glass nav: transparent over hero, `backdrop-filter:
  blur(20px) + bg rgba(10,9,8,0.85) + 1px ember-glow border` on
  scroll past 24px or when a mega-menu opens.
- Hover-triggered mega-menus: Products (3 product cards with
  geometric Console/Mobile/HQ marks + status badges), Company (3
  cols), Resources (3 cols). Customers + Pricing are direct links.
- Mobile drawer is full-screen, blur backdrop, stagger reveal via
  Framer; body scroll locks while open.
- Footer is Anthropic-grade: top wordmark + "Intelligence at work."
  + newsletter form (presentation-only); 5-col link grid (Praxis /
  Company / Engineering / Customers / Resources); bottom band with
  copyright + West Palm Beach + privacy/terms/AUP + X / GitHub /
  LinkedIn social row. Placeholder routes render as muted text
  with "soon" caption — depth without dishonest links.

### Phase 3 — homepage (`feat(home): homepage rebuild — hero + sections 1-8`)
Eight sections in order:
1. Hero — full viewport, ember radial top-right + mesh + 14 dust
   motes. "Stop hiring. / Start deploying." (Option A "hires zero
   / ships everything" preserved as comment for future A/B).
   Console preview on the right: full window chrome, sidebar with
   9 dept dots, user/Atlas chat with Engineering handoff bar, pill
   input. Lunaro mark + 2 ghost slots in the customer-marks row.
2. Three surfaces — Console / Mobile / HQ cards with Phosphor-
   stroke marks, status badges (Live / Beta · TestFlight / Q3
   2026), hover lift-4px + ember-glow ring.
3. Praxis HQ cinematic — 16:9 frame max-w 1100px with ember aura.
   Static fallback (Buildings duotone + ember pulse + grid mask)
   since `/public/videos/praxis-hq-preview.mp4` doesn't exist; the
   video tag is commented in the source for swap-in. 3 mono-font
   ticker captions stagger below.
4. The bet — single-column 720px; eyebrow + display-xl; 2-paragraph
   thesis; italic Fraunces pull-quote with ember vertical accent
   line; "Read the full thesis →" to /approach.
5. Customers — Lunaro card with 200+/6/9 stat row, Quotes icon,
   italic blockquote, 50/50 partnership tag. 2 ghost cards beside.
6. Praxis Engineering proof — sticky 2-col layout. Left: header
   stats (7m build/deploy, 100% real commits) + run-a-build CTA.
   Right: gradient-frame code block with real Server Component
   from the Lunaro pipeline build. "This is real. Generated by
   Praxis Engineering today." caption + deploy timestamp.
7. Pricing — 3 tiers (Free / Pro [popular, scaled 1.02 with ember
   ring] / Enterprise) with Phosphor checkmarks.
8. Final CTA — strong ember mesh + bottom ember aura. "Stop
   hiring. Start deploying." + dual CTAs + email-luis trust line.
- `Story.tsx` deleted (replaced by `EngineeringProof.tsx`).
- All sections respect `prefers-reduced-motion` via Framer
  `whileInView` + the CSS reduced-motion block from Phase 1.

### Phase 4 — product pages (`feat(products): all 4 product pages rebuild`)
- /products hub: 3 detailed cards (alternating text/preview
  sides), each with status badge, 2-paragraph body, 4 feature
  bullets, primary + secondary CTAs.
- /products/praxis-console: hero + Console preview + What it does
  (Talk/Build/Coordinate) + Meet the team (9 employees with
  per-dept colour dot + voice-sample affordance) + Behind the
  scenes (Voice/Memory/Execution metrics) + Built on row
  (Anthropic / Vercel / Supabase / ElevenLabs / LiveKit / GitHub).
- /products/praxis-mobile: hero with full PhoneMockup (Atlas-
  listening avatar, transcript bubble) + Voice-first +
  WaveformIllustration (36 ember bars) + Push +
  NotificationIllustration + waitlist with email + name + role
  select.
- /products/praxis-hq: hero + cinematic frame + 4 room cards
  (Marketing / Engineering / Boardroom / Atlas's office) + Spatial
  diagram (SVG hub-and-spoke) + 3-step timeline + early-access
  with email + agency + use-case select.
- WaitlistForm v2: extraFields config, packs to source string as
  `key=value|key=value::base_source`. /api/conduit/waitlist source
  cap bumped 64 → 256. No DB migration needed.

### Phase 5 — /about (`feat(about): /about company-energy rewrite`)
- Hero, Mission (3 paragraphs as company position), Approach (3
  principles cards), Customers (Lunaro + ghost), Contact (clean
  ContactRow grid: luis@ partnerships, press@ press, West Palm
  Beach HQ, working hours).
- Zero founder bio. The previous "Luis Garcia. 21." section is
  gone.

### Phase 6 — /approach (`feat(approach): /approach page (Linear /method energy)`)
- New page. Single column 720px. 5 numbered sections (01-05): On
  specialization, On memory, On voice, On execution, On the next
  decade. Each ~3 paragraphs body + italic Fraunces pull-quote
  with ember vertical accent line. Hairline ember dividers
  between sections. No images — type is the design.
- Closing CTA anchored to Open Praxis Console.

### Phase 7 — /customers (`feat(customers): /customers + /customers/lunaro`)
- /customers hub: hero, Lunaro card with 3 stats + 2 ghost slots,
  become-a-customer CTA card.
- /customers/lunaro: hero with eclipse-purple radial accent, 4-stat
  row, 4-paragraph story (Problem / Bet / Build / Result), 3
  highlight cards, Quotes block ("Quote pending Jonathan review"
  placeholder — honest about it), closing row with Read more →
  /products + See if Praxis fits → mailto.

### Phase 8 — /pricing (`feat(pricing): dedicated /pricing page`)
- Reuses homepage Pricing tier cards (single source of truth for
  tier copy/prices).
- Side-by-side comparison table: 14 features × 3 tiers, Pro column
  tinted ember at 4% opacity, footer row with per-tier CTA. Cell
  renderer handles boolean (Phosphor Check/Minus) + string values.
- 6-question FAQ (Top-ups / Praxis Depth / Multi-user / Cancel /
  Token cap / Custom enterprise) in <details>/<summary> with
  rotating + indicator.
- Custom enterprise CTA: mailto luis@ with subject pre-filled.

### Phase 9 — polish (`feat(polish): mobile + animations pass`)
- Skip-to-main link in layout (visible on focus).
- `scroll-padding-top: 96px` on html so anchor jumps clear the
  sticky nav.
- Mobile-tighter `.conduit-section` / `.conduit-hero-section` /
  `.conduit-container` overrides at <640px viewport.
- Removed reduced-motion's smooth-scroll override.
- Deleted 12 orphan components from the prior marketing era:
  Logo, Terminal, TerminalTyper, Constellation, ConstellationSVG,
  HeroParticles, HeadlineCycler, Departments, LayeredBuilding,
  LiveAgentGrid, PlatformTeaser, CaseStudy.
- `npm run build` clean. 10 marketing pages prerender static. /app
  routes still server-rendered as before.

### What's NOT verified end-to-end this round
- Visual / browser verification needs Luis on the conduitai.io
  Vercel preview. Push triggers a preview deploy.
- Lunaro waitlist form's packed `source` string requires a single
  POST in production to confirm Supabase RLS still accepts the
  ~200-char string. Hand-verified that the API and form types
  agree; the existing schema's `source text` column has no length
  constraint.
- Cinematic HQ section + product-page HQ frame both render the
