...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <motion.div
      ...
      className="conduit-card cursor-pointer"
    ...
    >
      <div className="p-6 md:p-7 flex items-start justify-between gap-4">
        ...
        <motion.span
          ...
          aria-hidden
          className="shrink-0 mt-1.5 w-6 h-6 rounded-full bg-[var(--color-ink-surface-elevated)] border border-[var(--color-edge)] flex items-center justify-center text-[var(--color-cream-mute)] hover:text-[var(--color-indigo-500)] hover:border-[var(--color-indigo-500)] transition-colors"
          style={{ fontSize: "14px" }}
        >
          +
        </motion.span>
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          <motion.dd
            ...
            className="px-6 md:px-7 text-[15px] md:text-[16px] text-[var(--color-cream-soft)] leading-[1.7]"
          >
            <div className="pb-6 md:pb-7">{a}</div>
          </motion.dd>
        )}
      </AnimatePresence>
    </motion.div>
