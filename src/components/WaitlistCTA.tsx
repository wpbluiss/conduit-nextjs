        ...
        {state === "success" ? (
          <div className="rounded-xl border border-orange/30 bg-orange/5 p-8">
            ...
            <p className="text-text2 text-sm">
              We will reach out when your slot opens. Expect to hear from us soon.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input
              ...
              required
              className="flex-1 px-4 py-3.5 rounded-full bg-card border border-border2 text-text font-[family-name:var(--font-mono)] text-sm placeholder:text-text3 focus:outline-none focus:border-white/20 focus:shadow-[0_0_20px_rgba(255,255,255,0.05)] transition-all"
            />
            <button
              type="submit"
              disabled={state === "loading"}
              className="px-8 py-3.5 rounded-full font-semibold text-sm bg-white text-black hover:bg-white/90 hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:hover:translate-y-0 whitespace-nowrap"
            >
              {state === "loading" ? (
        ...
        <div className="flex flex-wrap justify-center gap-6 mt-8">
          {["No credit card required", "Early access priority", "Cancel anytime"].map((item) => (
            <div key={item} className="flex items-center gap-2 text-text3 text-xs font-[family-name:var(--font-mono)]">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3.5 h-3.5 text-text2">
                <polyline points="20 6 9 17 4 12" />
              </svg>
              {item}
            </div>
