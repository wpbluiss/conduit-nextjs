...
export function NetWorthAreaChart({ data }: { data: { date: string; value: number }[] }) {
  return (
    <ResponsiveContainer width="100%" height={220}>
      <AreaChart data={data} margin={{ top: 8, right: 8, left: -10, bottom: 0 }}>
        <defs>
          <linearGradient id="finNW" x1="0" y1="0" x2="0" y2="1">
        ...
        <YAxis stroke="rgba(255,255,255,0.25)" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v) => `$${(v / 1000).toFixed(1)}k`} />
        <Tooltip
          contentStyle={{ background: "rgba(12,12,18,0.95)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, fontSize: 12 }}
          labelStyle={{ color: "#8c8884" }}
          formatter={(v: number | string) => `$${Number(v).toLocaleString()}`}
        />
        <Area type="monotone" dataKey="value" stroke="#ff8a3d" strokeWidth={2} fill="url(#finNW)" />
      </AreaChart>
    </ResponsiveContainer>
  );
}
