...
// GitHub redirects here after the user grants OAuth access.
// Validates CSRF state, exchanges the auth code for a token, and stores it.
export async function GET(request: NextRequest) {
  ...
  // Fetch user login and top repos to populate context automatically.
  const [userInfo, repos] = await Promise.all([
    fetchGithubUserLogin(tokens.access_token),
    fetchGithubUserRepos(tokens.access_token),
  ]);
    ...
    .upsert(
      {
        ...
        meta: {
          ...
          // Store top 3 repos by most recently updated for context injection.
