 ...
 * Step-2 interaction layer: ⌘K command palette, slash-commands, @mention
 * routing (pull any employee into the thread), message reactions + actions,
 * streaming replies, spring motion. Ember/PDL identity, Watermelon UI.
...
const THREADS: Record<string, Msg[]> = {
  c1: [
    ...
    { id: "m2", role: "assistant", emp: "engineering", content: "We have 50 tables with RLS disabled — that's the blocker. 38 are account-scoped, 8 public-read, 4 need custom policies. I can write the account-scoped batch now; the 4 custom ones I'd want you to confirm first. Want me to start?" },
  ],
  c2: [{ id: "m1", role: "user", content: "Give me a content plan for launch week." }, { id: "m2", role: "assistant", emp: "marketing", content: "Founder-led, 5 days: origin story, demo, a real result, behind-the-scenes, open invite. I'll draft all five; you record two videos. Voice or punchier?" }],
...
const REPLY: Record<EmpId, string> = {
  ...
  engineering: "Understood. I'll ship the safe parts now and tee up anything heavy as a brief for your Max session.",
  sales: "On it — I'll move that through the pipeline and come back with the next action and a date.",
  marketing: "Love it. I'll draft it in your voice, give you two angles, and queue the assets.",
...
export default function ChatPreview() {
  ...
  // menu derived from input: slash (leading "/") or @mention (trailing @token)
  ...
  function push(text: string) {
    ...
    setThinking(true);
    setTimeout(() => { setMessages((m) => [...m, { id: "a" + Date.now(), role: "assistant", emp: activeEmp, content: REPLY[activeEmp], stream: true }]); setThinking(false); }, 950);
  }
  ...
  const Rail = (
    <div className="flex h-full flex-col">
      ...
      <div className="px-3 pb-2">
        <button onClick={() => setPalette(true)} className="flex h-9 w-full items-center gap-2 rounded-lg border border-input bg-secondary/50 px-3 text-sm text-muted-foreground hover:bg-secondary">
          <Search className="size-4" /> Search<span className="ml-auto flex items-center gap-0.5 rounded border border-white/10 px-1.5 py-0.5 text-[10px] font-mono"><Command className="size-2.5" />K</span>
        </button>
      </div>
      ...
      <AnimatePresence>
        {palette && (
          <div className="fixed inset-0 z-[60] flex items-start justify-center p-4 pt-[12vh]">
            ...
            <motion.div initial={{ opacity: 0, y: -12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} transition={{ type: "spring", stiffness: 300, damping: 26 }}
              className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-white/10 bg-card wm-glow">
              <div className="flex items-center gap-2 border-b border-white/8 px-4">
                <Search className="size-4 text-muted-foreground" />
                <input autoFocus value={paletteQ} onChange={(e) => setPaletteQ(e.target.value)} placeholder="Jump to a teammate, start a chat…" className="h-12 flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" />
                <kbd className="rounded border border-white/10 px-1.5 py-0.5 text-[10px] text-muted-foreground">esc</kbd>
              </div>
              <div className="max-h-72 overflow-y-auto p-2">
                ...
                {ORDER.filter((id) => EMP[id].name.toLowerCase().includes(paletteQ.toLowerCase())).map((id) => {
                  const E = EMP[id];
                  return <button key={id} onClick={() => switchEmp(id)} className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left text-sm hover:bg-secondary"><span className="grid size-7 place-items-center rounded-md bg-secondary text-primary"><E.icon className="size-4" /></span><span className="flex-1">{E.name}</span><span className="text-xs text-muted-foreground">{E.role}</span></button>;
                })}
              </div>
            </motion.div>
          </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="no-scrollbar flex items-center gap-2 overflow-x-auto border-b border-white/8 px-4 py-2.5">
          ...
          {ORDER.map((id) => {
            ...
            return <button key={id} onClick={() => switchEmp(id)} className={`flex shrink-0 items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium transition-colors ${on ? "border-primary/50 bg-primary/15 text-foreground" : "border-white/8 text-muted-foreground hover:bg-secondary hover:text-foreground"}`}><E.icon className="size-3.5" />{E.name}</button>;
          })}
        </div>
        ...
        <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto">
          <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-7">
            ...
            {thinking && (
              <div className="flex gap-3"><span className="mt-0.5 grid size-8 shrink-0 place-items-center rounded-lg bg-secondary text-primary"><emp.icon className="size-4" /></span><div className="flex items-center gap-1 pt-2">{[0, 1, 2].map((i) => (<motion.span key={i} className="size-1.5 rounded-full bg-muted-foreground" animate={{ opacity: [0.3, 1, 0.3], y: [0, -2, 0] }} transition={{ duration: 1, repeat: Infinity, delay: i * 0.18 }} />))}</div></div>
            )}
            {showSuggest && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-wrap gap-2 pl-11">
                {SUGGEST[activeEmp].map((s) => <button key={s} onClick={() => push(s)} className="rounded-full border border-white/10 bg-secondary/50 px-3 py-1.5 text-sm text-muted-foreground transition-colors hover:border-primary/40 hover:bg-secondary hover:text-foreground">{s}</button>)}
              </motion.div>
            )}
          </div>
        </div>
        ...
        <div className="border-t border-white/8 bg-background/70 px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] backdrop-blur">
          <form onSubmit={(e) => { e.preventDefault(); push(input); }} className="relative mx-auto w-full max-w-3xl">
                ...
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }} className="absolute bottom-full left-0 mb-2 w-72 overflow-hidden rounded-xl border border-white/10 bg-card wm-glow">
                  ...
                  <div className="max-h-60 overflow-y-auto pb-1">
                    {menu === "slash" ? (slashItems.length ? slashItems : SLASH).map((s) => (
                      <button key={s.cmd} type="button" onClick={() => applySlash(s)} className="flex w-full items-center gap-3 px-3 py-2 text-left hover:bg-secondary">
                        <span className="grid size-7 place-items-center rounded-md bg-secondary text-primary"><s.icon className="size-4" /></span>
                        <span className="min-w-0 flex-1"><span className="block font-mono text-sm">{s.cmd}</span><span className="block truncate text-xs text-muted-foreground">{s.desc}</span></span>
                      </button>
                    )) : mentionItems.map((id) => { const E = EMP[id]; return (
                      <button key={id} type="button" onClick={() => applyMention(id)} className="flex w-full items-center gap-3 px-3 py-2 text-left hover:bg-secondary">
                        <span className="grid size-7 place-items-center rounded-md bg-secondary text-primary"><E.icon className="size-4" /></span>
                        <span className="min-w-0 flex-1"><span className="block text-sm">{E.name}</span><span className="block truncate text-xs text-muted-foreground">{E.role}</span></span>
                      </button>
                    ); })}
                  </div>
                </motion.div>
            ...
            <p className="wm-label mt-2 flex items-center justify-center gap-3">
              <span className="flex items-center gap-1"><Command className="size-3" />K palette</span>
              <span className="flex items-center gap-1"><Slash className="size-3" />commands</span>
              <span className="flex items-center gap-1"><AtSign className="size-3" />route</span>
            </p>
          </form>
        </div>
      </div>
